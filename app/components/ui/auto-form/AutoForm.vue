...
const shapes = computed(() => {
  ...
  Object.keys(shape).forEach((name) => {
    ...
    val[name as keyof T] = {
      ...
      options,
      required: !["ZodOptional", "ZodNullable"].includes(item._def.type),
      schema: baseItem,
    };
  });
  return val;
...
const formComponentProps = computed(() => {
  if (props.form) {
    return {
      onSubmit: props.form.handleSubmit((val) => emits("submit", val as any)),
    };
  } else {
    ...
    return {
      ...
      onSubmit: (val: GenericObject) => emits("submit", val as any),
    };
  }
});
</script>
